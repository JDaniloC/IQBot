...
class Catalogador(IQ_API): 
    ...
    def __init__(self, verboso = False):
        ...
        super().__init__("binifo4010@hagendes.com", "senha123")
        self.mudar_treino()
    ...
    def catalogar(self, timeframe, dias, porcentagem, martingale, 
        ...
        def cataloga(par, dias, timeframe):
            data = []
            datas_testadas = []
            time_ = datetime.utcnow() - timedelta(hours = 3)
            sair = False
            conta_erros = 0
            while sair == False:
...
Catalogando {dias} dias de M{timeframe} até {limite} sinais com {porcentagem}% até {martingale} gales das {inicio} - {final}
        ...
        for par in P['digital']:
            if P['digital'][par]['open'] == True:
                timer = int(datetime.utcnow() - timedelta(hours = 3))
                ...
                self.mostrar_mensagem(
                    f'Finalizado em {int(datetime.utcnow() - timedelta(hours = 3)) - timer} segundos.')
        ...
        for entrada in entradas:
            if entrada["timestamp"] > datetime.utcnow() - timedelta(hours = 3):
                contagem += 1
                if contagem < limite:
