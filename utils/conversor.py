...
def pegar_comando_taxas(original_text: str) -> dict:
    ...
    return {
        ...
        "ordem": direction,
        "timeframe": timeframe,
        "timestamp": datetime_brazil().timestamp()
    }
...
""")
        ...
        timeframe = "Padrão"
        if linha['timeframe'] != 0:
            timeframe = f"M{linha['timeframe']}"
        lista_entradas.append(f'''
📊 Ativo: {linha["par"]} <br>
📅 Dia: {"/".join(list(map(strDateHour, linha["data"])))} <br>
⏱ Hora: {":".join(list(map(strDateHour, linha["hora"])))} <br>
{'⬆' if direcao == "call" else '⬇'} Direção: {direcao.upper()} <br> 
⏰ Período: {timeframe}
''')
